...
the tiny fixture, over what this engine serves.

``tests/_helpers/a3b_sweep.py`` partitions the vocabulary by which engines
the capability rows name; this engine is not in those rows, so the buckets
are read through its own declaration (``NnterpEngine.components``: the
module-boundary taps of every registered family plus the interiors its
address table reaches). Every shared component must agree with the
reference engine on the read and on the intervention's downstream effect;
so must the two kernel arguments the rows call hooks-only (``delta_query``
/ ``delta_key``), which this engine reads off the same kernel call; the
fused-forward interiors only an nnsight engine serves read to their
declared widths (their identities are pinned in the interior suites); and
the three per-token DeltaNet faces the chunked prefill kernel never
materializes refuse by name, with the reason ``component_unavailable``.
Anti-vacuity is not optional: every write case also asserts the patched
logits moved.
"""
...
SERVED = NnterpEngine.components
SHARED = (
...
def _served(components: tuple[str, ...]) -> tuple[str, ...]:
    return tuple(c for c in components if c in SERVED)


def _unserved(components: tuple[str, ...]) -> tuple[str, ...]:
    return tuple(c for c in components if c not in SERVED)
...
# --------------------------------------------------------------------------- #
# 1. reads — every served shared component, in every block type it exists in
# --------------------------------------------------------------------------- #
...
@pytest.mark.parametrize(
    "component", _served(sweep.SHARED_ANY_STREAM + sweep.SHARED_LINEAR_ONLY)
)
...
@pytest.mark.parametrize(
    "component", _served(sweep.SHARED_ANY_STREAM + sweep.SHARED_FULL_ONLY)
)
...
def _write_both(component, layer, hooks_bundle, trace_bundle, unpatched):
    ...
    assert not torch.allclose(traced, unpatched["trace"], atol=sweep.ATOL), (
        f"nnsight_nnterp: the interchange at {where} left the logits unchanged"
    )


@pytest.mark.parametrize(
    "component", sweep.write_cases(_served(sweep.SHARED_LAYERLESS))
)
...
@pytest.mark.parametrize(
    "component",
    sweep.write_cases(_served(sweep.SHARED_ANY_STREAM + sweep.SHARED_LINEAR_ONLY)),
)
...
@pytest.mark.parametrize(
    "component",
    sweep.write_cases(_served(sweep.SHARED_ANY_STREAM + sweep.SHARED_FULL_ONLY)),
)
...
# --------------------------------------------------------------------------- #
# 3. the single-engine buckets: the kernel arguments agree, the fused
#    interiors are served, the per-token faces refuse by name
# --------------------------------------------------------------------------- #


@pytest.mark.parametrize("component", _served(sweep.HOOKS_ONLY))
def test_read_parity_kernel_arguments(hooks_qwen, nnterp_qwen, layers, component):
    """The tiled q/k the reference engine reaches by swapping the kernel
...
def test_write_parity_kernel_arguments(
    hooks_qwen, nnterp_qwen, layers, unpatched_logits, component
):
    delta_layer, _ = layers
    _write_both(component, delta_layer, hooks_qwen, nnterp_qwen, unpatched_logits)
...
@pytest.mark.parametrize("component", _served(sweep.NNSIGHT_ONLY))
...
def test_every_fused_interior_is_served():
    assert not _unserved(sweep.NNSIGHT_ONLY)


# --------------------------------------------------------------------------- #
# 4. the seam: what this engine does not serve refuses by name
# --------------------------------------------------------------------------- #
...
def test_the_unserved_are_exactly_the_per_token_deltanet_faces():
    assert set(_unserved(SHARED + sweep.HOOKS_ONLY + sweep.NNSIGHT_ONLY)) == {
        "delta_kv_mem",
        "delta_state_update",
        "delta_state",
    }
...
@pytest.mark.parametrize("component", _unserved(SHARED + sweep.HOOKS_ONLY))
def test_a_per_token_face_is_refused_by_name(nnterp_qwen, layers, component):
    ...
    assert component not in NnterpEngine.components


def test_the_declaration_is_the_boundaries_plus_the_address_table():
    """What the engine declares is exactly what some family taps at a
    module boundary plus what the address table reaches — every kind the
    resolver knows accounted for, and every addressed component a kind the
    resolver marks as having no boundary."""
    ...
    kinds = {
        ...
        for c in SHARED + sweep.HOOKS_ONLY + sweep.NNSIGHT_ONLY
    }
    for component, declared in kinds.items():
        boundary = bool(declared & {"in", "out"})
        addressed = component in components_addressed()
        assert (component in SERVED) == (boundary or addressed), component
