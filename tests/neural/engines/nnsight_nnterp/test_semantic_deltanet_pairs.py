...
Run over the eight tensors that carry one name and a retired second spelling
(``delta_*``, with ``deltanet_*`` folded onto it at parse): the **same parsed
...
engine (hooks and kernel-global swaps) and this engine (envoys and
...
def test_one_document_one_intervention_two_engines(
    ...
    traced = _logits(NnterpExecutor, raw, nnterp_qwen)
...
def test_each_engine_translates_the_name_to_its_own_mechanism(
    ...
    trace_site = resolve_site(nnterp_qwen, doc_site(pair.hooks, delta_layer))
    ...
    address = executor._address("the site", trace_site)  # pyright: ignore[reportPrivateUsage]
    if site.kind == "delta":
        # the reference engine swaps a kernel global; this engine drills a
        # `.source` line from the mixer envoy the shared resolver anchored
        assert address is not None and address.path
        assert trace_site.module is nnterp_qwen.mixer_at(delta_layer)
    else:
        # a module boundary: a hook on one engine, an envoy on the other
        assert site.kind in ("in", "out") and address is None
