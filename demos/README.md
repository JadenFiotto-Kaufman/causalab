...
[Saved hypothesis comparisons](hypothesis_testing/hypothesis_testing.md) checks
the handoff from exact pairs to symbolic predictions and saved neural outputs.
...
each one is in the demo's markdown, and the demo's workflow pins the table by
digest in its `pins` section (workflow spec §7).
