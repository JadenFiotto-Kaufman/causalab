...
| **Data** | `mcqa/data#train` to fit the basis, `mcqa/data#test` to score it |
...
**The two arms share one intervention specification.** `apply32` is `apply1` with a `set`
...
```json
{
  ...
  "data": {"base": {"dataset": "mcqa/data#train", "field": "input"}},
  "method": {
...
# OK: demos/onboarding_tutorial/workflows/mcqa_pca.json — 6 steps
```
...
#   harvest: intervention_protocol ../protocols/mcqa_harvest.json — 1 point(s), campaign digest 43f4e4083e47b7cb…
...
#   apply1: intervention_protocol ../protocols/mcqa_pca_apply.json — 1 point(s), authored digest 881f2de47bc429e3…
#   apply32: intervention_protocol ../protocols/mcqa_pca_apply.json — 1 point(s), authored digest 64a2e3330f6e2ce5…
...
> `mcqa/data#train` and both applications on `mcqa/data#test`, which are different
...
- The comparison to 0.969 crosses splits. 03's full-vector number is on
  `pairs_n64_s0` and these are on `mcqa/data#test`, so it is a reference point
