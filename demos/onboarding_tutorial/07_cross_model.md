...
| **Reproduced** | ✓ 2026-08-31, `pytorch_hooks` on one H100 80GB; one workflow ran and its twin was refused as designed |
...
```json
{
  ...
  "method": {
    ...
    "reads": {"acts": {"site": "target", "pos": "slot", "model": "original", "input": "base"}},
    "save": [
      {
        "value": "acts",
        "model": "original",
...
```json
{
  ...
  "method": {
    ...
    "reads": {"logits": {"site": "lm_head", "pos": -1, "model": "grafted", "input": "base"}},
    "writes": {
...
# OK: demos/onboarding_tutorial/workflows/mcqa_cross_model.json — 2 steps
```

```bash
uv run causalab validate demos/onboarding_tutorial/workflows/mcqa_cross_model_refused.json \
    --data-root demos/onboarding_tutorial/data
# OK: demos/onboarding_tutorial/workflows/mcqa_cross_model_refused.json — 2 steps
```
...
#   level 0: source
#   level 1: graft
#   source: intervention_protocol ../protocols/mcqa_source_harvest.json — 1 point(s), campaign digest 5a2568891137b4b4…
#   graft: intervention_protocol ../protocols/mcqa_cross_patch.json — 1 point(s), authored digest 74145786d9c18b32…
```
