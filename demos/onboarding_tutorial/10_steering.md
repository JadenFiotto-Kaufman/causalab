...
| **Data** | `mcqa/pairs_n64_s0` to ablate, `mcqa/data#train` to build the direction, `mcqa/data#test` to steer |
...
```json
{
  ...
  "data": {"base": {"dataset": "mcqa/pairs_n64_s0", "field": "input"}},
  "method": {
...
```json
{
  ...
  "model": {"key": "meta-llama/Llama-3.2-1B-Instruct", "revision": "main", "dtype": "bf16"},
  "data": {"base": {"dataset": "mcqa/data#test", "field": "input"}},
  "method": {
    "positions": {"slot": {"index": -1}},
    "sites": {
      "target": {"component": "block_output", "layers": [14]},
    ...
    "reads": {"logits": {"site": "lm_head", "pos": -1, "model": "steered", "input": "base"}},
    "writes": {
      "push": {
        "site": "target",
        "pos": "slot",
...
# OK: demos/onboarding_tutorial/workflows/mcqa_steering.json — 6 steps
```
...
#   level 2: steer
#   level 3: steer_curve
#   ablate: intervention_protocol ../protocols/mcqa_ablate.json — 16 point(s), campaign digest f38a4bf8aa521d35…
#   ablation_curve: script causalab.io.plots.workflow_figures -> ablation.json, ablation.png
#   harvest: intervention_protocol ../protocols/mcqa_contrast_harvest.json — 1 point(s), campaign digest 7c369a7eec4a4c04…
#   direction: script causalab.analysis.harvest_difference -> direction.safetensors, stats.json
#   steer: intervention_protocol ../protocols/mcqa_steer.json — 11 point(s), authored digest 2b4f55a1a84c2858…
...
Run on 2026-08-31, one H100 80GB, reference engine (`pytorch_hooks`), bf16. All six steps completed.
...
- The ablation and the steering use different splits (`pairs_n64_s0` and
  `mcqa/data#test`), so their accuracies — 0.891 and 0.953 — are not directly
