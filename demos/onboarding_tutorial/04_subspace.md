...
| **Data** | fit on `mcqa/data#train` (128 pairs), score on `mcqa/data#test` (64 pairs) |
...
```mermaid
flowchart LR
  F["fit<br/>8 rotations, 20 epochs each<br/>mcqa/data#train"] -->|"rot.safetensors"| A["apply<br/>8 scores<br/>mcqa/data#test"]
...
| `train.eval` | `{"split": "mcqa/data#test", …}` | writes `train_eval.json` beside the fit — the eval-split score per epoch, which is not the same thing as this document's `iia.json` |
...
# OK: demos/onboarding_tutorial/workflows/mcqa_subspace.json — 3 steps
```
...
#   level 1: apply
#   level 2: test_curve
#   fit: intervention_protocol ../protocols/mcqa_das_fit.json — 8 point(s), campaign digest 4fa7cbe5f653d17d…
#   apply: intervention_protocol ../protocols/mcqa_das_apply.json — 8 point(s), authored digest 6154810ba0a6143a…
...
Run on 2026-08-31, one H100 80GB, reference engine (`pytorch_hooks`), bf16. All three steps completed; the fit trained
